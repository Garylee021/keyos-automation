# KeyOS Redundant VPN Server on Azure
...
- `variables.tf`: Access information, network parameters, KeyOS image parameters, and virtual parameters are defined here. You may edit/change these parameters based on your requirements. **Note:** After editing IP addresses inside `variables.tf`, check `files/keyos_user_data.tpl` file as well. All necessary configurations are based on this file.
...
- `main.tf`: Contains all parameters of the KeyOS VM (NVA) resources.

- `files/keyos_user_data.tpl` : All necessary configurations of the KeyOS have been defined in this file based on KeyOS cli reference.
...
For management, use `VPN_Server_Public_IP_Address` and KeyOS credentials from `outputs`.

Command sample:
```sh
ssh keyos@<VPN_Server_Public_IP_Address>
```

P.S. With the public address, you can manage only the `active` KeyOS instance. If you want to manage both instances at the same time, you need to initiate a remote access VPN connection via WireGuard software (by uploading the client profile file from the profiles folder). After a successful connection, you can manage instances using `KeyOS_01_Private_IP_Address` and `KeyOS_02_Private_IP_Address` from `outputs`.

```sh
ssh keyos@<KeyOS_01_Private_IP_Address>
ssh keyos@<KeyOS_02_Private_IP_Address>
```
