# KeyOS Deployment with Advanced Configuration

## Overview
This manual guides the deployment of a KeyOS instance in AWS using CloudFormation. The template sets up:
...
- **EC2 Key Pair**: Valid SSH key for accessing KeyOS.
...
#### KeyOS Instance Parameters
- **InstanceType**: EC2 instance type (e.g., t3.medium)
- **KeyName**: Name of EC2 KeyPair
- **KeyOSPublicENIip**: Private IP in the public subnet
- **KeyOSPrivENIip**: Private IP in the private subnet
- **KeyOSBGPASNumber**: BGP ASN for KeyOS
- **DNS1/DNS2**: Primary/Secondary DNS IP
- **SSHAllowedIP**: CIDR for SSH access (e.g., 192.0.2.0/24)
- **AmiAlias**: Specify KeyOS AMI alias (e.g., `latest` or version `/aws/.../1.4.0`)
...
7. **KeyOS Instance**: Configured via cloud-init with specified AMI and instance type
...
4. Configure **KeyOS Instance Parameters**
5. Add on-prem KeyOS parameters and finish deployment
...
2. Complete **KeyOS Instance Configuration**
3. Confirm deployment and retrieve Public IP from **Outputs**

## Access and Management
Use SSH to connect:
```bash
ssh keyos@<KeyOS_Public_IP_Address> -i <test-key.pem>
```

## Common Commands for KeyOS Management
...
For additional commands and configurations, refer to the [KeyOS Documentation](https://docs.keyos.io/en/sagitta/).
