# KeyOS Deployment with Basic Configuration

## Template Overview

This CloudFormation template automates the deployment of a KeyOS instance, setting up:
...
6. Configure KeyOS Instance parameters:
   - **Instance Type**.
   - **EC2 Key Pair Name**.
   ...
   - **SSH Allowed IP Subnet** (for remote access).
    ...
    > **Note**: Setting `KeyOS AMI Alias` to `latest` will deploy the latest version. Specify a specific version if needed, e.g., `/aws/service/marketplace/prod-ev235jujteaom/1.4.0`.
...
6. Configure KeyOS Instance parameters as listed above.
7. Wait for **CREATE_COMPLETE** and find the Public IP under “Outputs”.

## Access and Management

To connect to the KeyOS instance, use:
- **KeyOS Public IP** (from Outputs) and **EC2 Key Pair** with an SSH client.

Sample command: `ssh keyos@<KeyOS_Public_IP_Address> -i <test-key.pam>`

### Common CLI Commands

For KeyOS configuration and interface checking:
```bash
