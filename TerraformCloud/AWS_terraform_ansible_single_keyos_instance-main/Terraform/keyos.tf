...
#
# In the script keyos.tf we'll use default values (you can chang it as you need)
# AWS Region = "us-east-1"
# AMI        = "standard AMI of KeyOS from AWS Marketplace"
# Size of VM = "t2.micro"
# AWS Region = "us-east-1"
...
variable "ami" {
 ...
 description = "Amazon Machine Image ID for KeyOS"
}
...
# my resource for KeyOS

resource "aws_instance" "myKeyOSec2" {
 ami = var.ami
 key_name = "awsterraform"                                      # Please enter your details from 1.2 of Preparation steps for deploying KeyOS on AWS 
 security_groups = ["awsterraformsg"]                           # Please enter your details from 1.3 of Preparation steps for deploying KeyOS on AWS 
...
# aws_instance.myKeyOSec2.public_ip - the information about public IP address
# of our instance, needs for provisioning and ssh connection from Ansible
##############################################################################

output "my_IP"{
value = aws_instance.myKeyOSec2.public_ip
}
...
resource "local_file" "ip" {
    content  = aws_instance.myKeyOSec2.public_ip
    filename = "ip.txt"
}
...
depends_on = [aws_instance.myKeyOSec2] 
connection {
 type     = "ssh"
 user     = "root"
