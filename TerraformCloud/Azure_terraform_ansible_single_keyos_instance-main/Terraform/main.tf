...
# An KeyOS server without ssh key (only login+password)
##############################################################################
...
resource "azurerm_resource_group" "azure_keyos" {
...
##############################################################################
# Build an KeyOS VM from the Marketplace
# To finde nessesery image use the command:
#
# az vm image list --offer keyos --all
#
# Now that we have a network, we'll deploy an KeyOS server.
...
resource "azurerm_network_security_group" "keyos-sg" {
  name                = "${var.prefix}-sg"
  location            = "${var.location}"
  resource_group_name = "${var.resource_group}"
...
resource "azurerm_network_interface" "keyos-nic" {
  name                      = "${var.prefix}keyos-nic"
  location                  = "${var.location}"
  resource_group_name       = "${var.resource_group}"
  ...
  ip_configuration {
    ...
    public_ip_address_id          = "${azurerm_public_ip.keyos-pip.id}"
  }
}

# Add a public IP address.

resource "azurerm_public_ip" "keyos-pip" {
  name                         = "${var.prefix}-ip"
  location                     = "${var.location}"
  resource_group_name          = "${var.resource_group}"
...
# Build a virtual machine. This is a standard KeyOS instance from Marketplace.

resource "azurerm_virtual_machine" "keyos" {
  name                = "${var.hostname}-keyos"
  location            = "${var.location}"
  resource_group_name = "${var.resource_group}" 
  vm_size             = "${var.vm_size}"

  network_interface_ids         = ["${azurerm_network_interface.keyos-nic.id}"]
...
# az vm image list --offer keyos --all
  ...
  plan {
    ...
    name      = "keyos-1-3"
    product   = "keyos-1-2-lts-on-azure"
  }
...
data "azurerm_public_ip" "example" {
  depends_on = ["azurerm_virtual_machine.keyos"]
  name                = "keyos-ip"
  resource_group_name = "${var.resource_group}"
}
